    ...
    return (
        <form className="space-y-6 bg-white p-6 rounded-lg shadow-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label className="label"><span className="label-text">Chairperson</span></label>
                    <input type="text" placeholder="Chairperson" defaultValue="Steve Jobs" className="input input-bordered w-full" />
                </div>
                <div>
                    <label className="label"><span className="label-text">Host</span></label>
                    <input type="text" placeholder="Host" defaultValue="Mamun Hasan" className="input input-bordered w-full" />
                </div>
                <div>
                    <label className="label"><span className="label-text">Participants</span></label>
                    <input type="text" placeholder="Participants" defaultValue="Sumaiya, Salam, Ibrahim" className="input input-bordered w-full" />
                </div>
            </div>
                    ...
                    {agendaItems.map((item, idx) => (
                        <div key={idx} className="collapse collapse-arrow border border-base-300 rounded-box">
                            ...
                            <div className="collapse-title text-md font-medium">
                                {item.agenda || `Agenda ${idx + 1}`}
                            </div>
                            <div className="collapse-content space-y-4">
                                {/* Agenda Inputs */}
                                <div className="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
                                    <div className="md:col-span-3">
                                        <label className="label"><span className="label-text">Agenda</span></label>
                                        <input type="text" defaultValue={item.agenda} className="input input-bordered w-full" />
                                    </div>
                                    <div className="md:col-span-2">
                                        <label className="label"><span className="label-text">Owner</span></label>
                                ...
                                <div className="space-y-3 pt-2">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <button type="button" className="btn btn-xs btn-outline" onClick={() => handleAddAction(idx)}>
                                            <FiPlus className="mr-1" /> Add Action
                                        </button>
                                    </div>
                                    ...
                                    {item.actions.map((action, aIdx) => (
                                        <div key={aIdx} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end border p-3 rounded-md">
                                            ...
                                            <div className="md:col-span-1 flex items-center justify-center pt-6">
                                                <button type="button" onClick={() => handleRemoveAction(idx, aIdx)} className="btn btn-sm btn-circle btn-error btn-outline">
                                                    <FiTrash2 />
                                                </button>
                                            </div>
                                        </div>
            ...
            {/* Notes */}
            <div>
                <label className="label"><span className="label-text">Meeting Notes / Summary</span></label>
                <textarea className="textarea textarea-bordered w-full" rows={5} placeholder="Write meeting notes, decisions, and summary..."></textarea>
            </div>
